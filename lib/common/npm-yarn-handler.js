...
class NpmYarnHandler {
  static install() {
    Utils.print(chalk.bold.underline.magenta("- install packages"));
    shell.exec("yarn");
    Utils.printDone();
  }
  ...
  static upgrade(opt_key, opt_path) {
    if (!opt_key) {
      Utils.print(chalk.bold.underline.magenta("- upgrade packages"));
      shell.exec("yarn upgrade");
    } else {
      Utils.print(chalk.bold.underline.magenta("- upgrade " + chalk.italic(opt_key)));
      shell.exec("yarn upgrade " + opt_path);
    }
    Utils.printDone();
  }
  ...
  static releaseDry() {
    Utils.print(chalk.bold.underline.magenta("- release dry-run (using standard-version)"));
