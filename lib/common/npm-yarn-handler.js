...
class NpmYarnHandler {
  static install() {
    Utils.printYarn("install/update packages");
    shell.exec("yarn");
    Utils.printDone();
  }
  ...
  static watch(repo) {
    ...
    shell.exec("ttab -t ◉_◉" + repo + "◉_◉ -G exec npm run watch")
  }

  static dev(repo, type) {
    Utils.printYarn("start dev server in new tab  ( ͡° ͜ʖ ͡°)");
    shell.exec("ttab -t ◉_◉" + repo + "◉_◉ -G npm run dev" + (type ? ':' + type : ''));
  }
  ...
  static releaseDry() {
    ...
    shell.exec("standard-version --dry-run");
    Utils.printDone();
  }
