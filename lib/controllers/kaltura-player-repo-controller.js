...
class KalturaPlayerController {
  ...
  startDevMode() {
    this.keys.forEach((key) => {
      if (this.config[key][constants.DEV_MODE][constants.LINK]) {
        NpmYarnRepoController.linkRepo(key);
      }
    });
    NpmYarnRepoController.dev(constants.KALTURA_PLAYER_JS);
  }
  ...
  stopDevMode() {
    try {
      ...
      this.keys.forEach((key) => {
        if (this.config[key][constants.DEV_MODE][constants.LINK]) {
          NpmYarnRepoController.unlinkRepo(key, pkgJsonDependencies[key]);
        }
      });
      NpmYarnRepoController.build();
    }
    catch (e) {
