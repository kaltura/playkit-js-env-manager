...
const changelogParser = require("../common/changelog-parser");
...
function getNewVersion() {
  ...
  utils.print(chalk.bold.underline.blue("- extract new version:") + chalk.yellow(" " + newVersion));
  utils.printDone();
  return newVersion;
}
...
function cancelReleaseMode() {
  /**
   * TODO: replace to resetHardMaster and delete resetHardDevelop
   */
  // resetHardMaster();
  resetHardDevelop();
  deleteLocalTag();
...
function resetHardDevelop() {
  utils.print(chalk.bold.underline.blue("- reset hard"));
  shell.exec("git reset --hard origin/develop");
  utils.printDone();
}

function resetHardMaster() {
  utils.print(chalk.bold.underline.blue("- reset hard"));
...
function deleteLocalTag() {
  ...
  shell.exec("git tag -d v" + _releaseData[_repo][constants.VERSION]);
  utils.printDone();
}
